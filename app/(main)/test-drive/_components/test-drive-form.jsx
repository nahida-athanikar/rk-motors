...
const TestDriveForm = ({ car, testDriveInfo }) => {
  ...
  // Helper: minutes -> "HH:MM" (zero padded)
  ...
  // Helper: check overlapping intervals (in minutes)
  const isOverlappingMinutes = (bStartMin, bEndMin, sStartMin, sEndMin) => {
    return bStartMin < sEndMin && sStartMin < bEndMin;
  };
  ...
  useEffect(() => {
    if (bookingResult?.success) {
      setBookingDetails({
        ...
        timeSlot: `${format(
          new Date(`2022-01-01T${bookingResult?.data?.startTime}`),
          "h:mm a"
        )} - ${format(
          new Date(`2022-01-01T${bookingResult?.data?.endTime}`),
          "h:mm a"
        )}`,
  ...
  useEffect(() => {
    ...
    if (!daySchedule || !daySchedule.isOpen) {
     ...
     // Parse opening and closing hours (expect "HH:mm")
    const openMin = toMinutes(daySchedule.openTime);
    const closeMin = toMinutes(daySchedule.closeTime);

    if (Number.isNaN(openMin) || Number.isNaN(closeMin) || openMin >= closeMin) {
      setAvailableTimeSlots([]);
      return;
    }

     // Generate time slots (every 60 minutes, aligned to open time)
    const slots = [];
    for (let startMin = openMin; startMin + 60 <= closeMin; startMin += 60) {
      const endMin = startMin + 60;

      const startTime = minutesToTime(startMin);
      const endTime = minutesToTime(endMin);

      // Check if this slot is already booked (using minutes for robust comparison)
      const isBooked = existingBookings.some((booking) => {
        const sameDate = booking.date === format(selectedDate, "yyyy-MM-dd");
        if (!sameDate) return false;

        const bStartMin = toMinutes(booking.startTime);
        const bEndMin = toMinutes(booking.endTime);
        if (Number.isNaN(bStartMin) || Number.isNaN(bEndMin)) return false;

        return isOverlappingMinutes(bStartMin, bEndMin, startMin, endMin);
      });
  ...
  }, [selectedDate, dealership, JSON.stringify(existingBookings)]);

   // Create a function to determine which days should be disabled
  const isDayDisabled = (day) => {
    if (!day) return true;

    // Disable past dates (allow selecting today)
